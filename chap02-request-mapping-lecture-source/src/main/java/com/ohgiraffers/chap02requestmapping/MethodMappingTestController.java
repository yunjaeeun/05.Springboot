...
public class MethodMappingTestController {
    ...
    /* 설명. 핸들러 메소드(Annotation을 활용해서 요청 방식 및 경로에 따라 각각 메소드가 작성 됨. */
    @RequestMapping(value = "/menu/regist")         // GET 요청 뿐 아니라 다른 요청 방식도 처리됨
//    @RequestMapping(value = "/menu/regist", method = RequestMethod.GET)
    public String registMenu(Model model) {                 // Model -> 응답할 페이지의 재료를 담는 객체이다.
        System.out.println("/menu/regist 경로의 GET 요청 받아보기");

        model.addAttribute("message", "신규 메뉴 등록용 핸들러 메소드 호출함");     // 키와 밸류 형태로 값을 저장

        /* 설명. 핸들러 메소드에서 반환하는 String 값은 templates에 있는 view(html 파일)의 이름이다. */
        return "mappingResult";
    }

    @RequestMapping(value = "/menu/modify")         // GET 요청 뿐 아니라 다른 요청 방식도 처리됨
