...
//    public String registMenu(HttpServletRequest request, Model model) {
    public String registMenu(WebRequest request, Model model) {              // Spring에서 권장하는 방법
     ...
     * */
    @PostMapping("modify")
    public String modiftMenu(Model model,
                             @RequestParam(defaultValue = "디폴트", name = "name") String modifyName
            , @RequestParam(defaultValue = "0") int modifyPrice) { // Integer.valueOf(request.getParameter("modifyPrice"))와 같은 의미
        String message = modifyName + " 메뉴의 가격이 " + modifyPrice + "원으로 변경되었습니다.";
        model.addAttribute("message", message);
    ...
    @PostMapping("modify2")
    public String modiftMenu(Model model, @RequestParam Map<String, String> parameter) {

        String modifyName = parameter.get("name2");
        int modifyPrice = Integer.valueOf(parameter.get("modifyPrice2"));
        String message = modifyName + " 메뉴의 가격이 " + modifyPrice + "원으로 변경되었습니다.";
        model.addAttribute("message", message);         // model에 담아 message라는 키값을 가지고 전달.
    ...
    /* 설명.
     *  (커맨드 객체)
     *  핸들러 메소드에 우리가 작성한 클래스를 매개변수로 작성하면 스프링이 객체를 만들어 주고 setter로 값도 주입해 준다.
     *
     * 설명.
     *  @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키값을 지정할 수 있다.
     *  (키 값이 없으면 타입의 낙타봉 표기법이 키값)
    * */
    /* 설명. @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키값을 지정할 수 있다.(키 값이 없으면 타입의 낙타봉 표기법이 키값) */
    @PostMapping("search")
    public String searchMenu(@ModelAttribute("menu") MenuDTO menu){     // 메소드가 호출되는 순간 기본 생성자로 객체를 생성 하고 Setter로 호출해 형변환후 자료를 넣어줌(= new MenuDTO())
    ...
    public String sessionTest1(HttpSession session, @RequestParam String id){
        ...
        return "first/loginResult";         // loginResult에서 모델에 담아 꺼내는게 아닌 Session에 담긴 값을 서버에서 꺼냄.
    }
